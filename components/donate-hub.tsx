      ...
      {KAPOETA_GOALS.map((goal) => {
        ...
        return (
          ...
          >
            ...
            <p className="text-[#8C7B72] text-sm leading-relaxed mb-5 flex-grow">{goal.short}</p>
            ...
            <div className="mb-6">
              <div className="flex items-end justify-between mb-2">
                ...
                <span className="text-xs text-[#8C7B72]">
                  of {formatAUDFull(goal.goalAmount)}
                </span>
              </div>
              ...
              <div className="flex items-center justify-between mt-2">
                <span className="text-xs font-semibold" style={{ color: "#C9952A" }}>{pct}% funded</span>
                {supporters > 0 && (
                  <span className="text-xs text-[#8C7B72]">{supporters.toLocaleString()} supporters</span>
                )}
              </div>
            </div>
            ...
            >
              {goal.kind === "bundle" ? "See the breakdown" : "Donate"} <ArrowRight size={16} />
            </Link>
          </motion.div>
        );
      })}
